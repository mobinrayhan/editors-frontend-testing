...
const DarkLightMode = ({ className }) => {
    ...
    const isDarkMode = hasMounted && window.matchMedia("(prefers-color-scheme: dark)").matches;
    ...
    useEffect(() => {
        ...
        dispatch(changeSkin(skinMode === "auto" ? (isDarkMode ? "dark" : "light") : skinMode));
    ...
    return (
        <Fragment>
            {hasMounted && <Dropdown className={"ms-2 " + className} onSelect={changeSkinMode}>
                <Dropdown.Toggle
                    as="a"
                    bsPrefix=' '
